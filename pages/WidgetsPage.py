...
class WidgetsPage:
    ...
    class Slider(BasePage):
        ...
        def checking_slider_value(self, locator, element):
            elements = self.base_element.find_element_(
                self.browser, locator, element)
            value = int(self.base_element.get_to_attribute(element=elements, attribute="value"))-25
            return abs(value)
    ...
    class ProgressBar(BasePage):
        ...
        def wait_while_progress_bar_became(self, value_percent, locator, element):
            value_of_1_to_4 = 4
            correcting_value = 2
            if value_percent >= value_of_1_to_4:
                value = value_percent - correcting_value
                return self.correcting_value_percent(value_percent, value, locator, element)
            else:
                return self.correcting_value_percent(value_percent, value_percent, locator, element)

        def correcting_value_percent(self, value_percent, value, locator, element):
